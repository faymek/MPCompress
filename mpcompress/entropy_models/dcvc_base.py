...
class DmcCompressionModel(nn.Module):
    def __init__(self, z_channel, extra_qp=0):
        super().__init__()

        self.z_channel = z_channel
        self.entropy_coder = None
        self.bit_estimator_z = VbrFactorizedPrior(64 + extra_qp, z_channel)
    ...
    def get_y_gaussian_bits(self, y, sigma):
        ...
        return DmcCompressionModel.probs_to_bits(probs)
    ...
    def get_y_gaussian_likelihood(self, y, sigma):
        mu = torch.zeros_like(sigma)
        sigma = sigma.clamp(0.11, 64.0)
        gaussian = torch.distributions.normal.Normal(mu, sigma)
        probs = gaussian.cdf(y + 0.5) - gaussian.cdf(y - 0.5)
        return probs
    ...
    def update(self, force_zero_thres=None):
        ...
        self.gaussian_encoder.update(
            self.entropy_coder, force_zero_thres=force_zero_thres
        )
    ...
    def get_mask_2x(self, batch, channel, height, width, dtype, device):
        ...
        with torch.no_grad():
            if curr_mask_str not in self.masks:
                assert channel % 2 == 0
                m = torch.ones(
                    (batch, channel // 2, height, width), dtype=dtype, device=device
                )
    ...
    def compress_prior_2x(self, y, common_params, y_spatial_prior):
        y, q_dec, scales, means = self.separate_prior_for_video_encoding(
            common_params, y
        )
        dtype = y.dtype
        device = y.device
    ...
    def decompress_prior_2x_part2(self, common_params, y_spatial_prior, infos):
        ...
        y_q_r = self.gaussian_encoder.get_y(
            infos["scales_r"].shape,
        ...
        y_hat_0, cat_params = restore_y_2x_with_cat_after(
            y_q_r, infos["means"], infos["mask_0"], common_params
        )
        scales, means = y_spatial_prior(cat_params).chunk(2, 1)

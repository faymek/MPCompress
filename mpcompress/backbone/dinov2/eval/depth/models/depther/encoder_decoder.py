...
class DepthEncoderDecoder(BaseDepther):
    ...
    def whole_inference_encode(self, img, img_meta, rescale, size=None):
        """Feature extraction with full image."""
        x = self.extract_feat(img) 
    ...
    def whole_inference_decode(self, outputs, backbone_model, reshape, img_meta, rescale, shape=None, return_class_token=True, size=None):
        """Accuracy evaluation with full image.""" 
    ...
    def inference_encode(self, img, img_meta, rescale, size=None):
        """Feature extraction with slide/whole style.
        Args:
            img (Tensor): The input image of shape (N, 3, H, W).
            img_meta (dict): Image info dict where each dict has: 'img_shape',
                'scale_factor', 'flip', and may also contain
    ...
    def inference_decode(self, feature_list, backbone_model, img_meta, rescale,shape, size=None):
        """Accuracy evaluation with slide/whole style."""
        assert self.test_cfg.mode in ["slide", "whole"]
        ori_shape = img_meta[0]["ori_shape"]
        assert all(_["ori_shape"] == ori_shape for _ in img_meta)
        if self.test_cfg.mode == "slide":
    ...
    def aug_test_encode(self, imgs, img_metas, rescale=True):
        """Original feature extraction (backbone) with augmentations. Only rescale=True is supported."""
        # aug_test rescale all imgs back to ori_shape for now
        assert rescale
        # to save memory, we get augmented depth logit inplace
        feature_list=[]
    ...
    def aug_test_decode(self, features_list,backbone_model, img_metas, rescale=True,shape=None):
        """Accuracy evaluation (head) with augmentations. Only rescale=True is supported."""
        # aug_test rescale all imgs back to ori_shape for now
        assert rescale
        # to save memory, we get augmented depth logit inplace

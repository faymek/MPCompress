...
class FeatCompression(CompressionModel):
    def __init__(self,feat_dims, N=256,M=128):
        '''
...
class MLoRE(nn.Module):
    ...
    def forward(self, x, episode_tasks=None, eval=True):
        '''
        e.g.,
        ...
        bpp_loss, mse_loss, feat_hat = self.compress(last_feat[task])
        info['bpp_loss'] = bpp_loss
        info['mse_loss'] = mse_loss
        
        for task in all_tasks:
            last_feat[task] = feat_hat.clone()

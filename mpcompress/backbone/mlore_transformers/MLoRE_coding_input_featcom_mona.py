...
from mpcompress.backbone.mlore_transformers.attention import SATransformerBlock
import numpy as np
...
class Mona(BaseModule):
    ...
    def forward(self, x, hw_shapes=None):
        ...
        h, w = hw_shapes
        project1 = project1.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        project1 = self.adapter_conv(project1)
        project1 = project1.permute(0, 2, 3, 1).reshape(b, n, c).contiguous()
...
class Block(nn.Module):
    ...
    def __init__(self, chan_nheads, resolution, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, mona=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.mona:
            assert x.shape[1] == self.res[0]*self.res[1]
            x = self.my_module_1(x, self.res)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        if self.mona:
            x = self.my_module_2(x,self.res)  # todo: correct
...
class FeatCompression(CompressionModel):
    ...
    def forward(self, x, target=None):
        ...
        #print(torch.abs(y_hat-y_hat2).sum())
...
class MLoRE(nn.Module):
    ...
    def __init__(self, p, select_list, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, mona=(i==self.select_list[0]-1))
    ...
    def get_features(self,x):
        ...
        features = []
        self.return_feature_idx = self.select_list[0]
    ...
    def cal_task_feature(self, x, attn_weight, il, info, episode_tasks, prompts):
        ...
        route_prob_out1 = {''.join(tasks): 0 for tasks in episode_tasks}
        route_prob_out2 = {''.join(tasks): 0 for tasks in episode_tasks}
        for tasks in episode_tasks:
            input_prompt = [prompts[t] for t in tasks]
            input_prompt = torch.stack(input_prompt, dim=1)
            x_q[''.join(tasks)], route_feat_out[''.join(tasks)], route_prob_out1[''.join(tasks)], new_prompts = self.MLoRE_1(combined_fea, tasks, input_prompt)
            x_q[''.join(tasks)], route_feat_out[''.join(tasks)], route_prob_out2[''.join(tasks)], new_prompts = self.MLoRE_2(x_q[''.join(tasks)], tasks, new_prompts)
            for i, t in enumerate(tasks):
                prompts[t] = new_prompts[:,i].clone()

        combined_fea = x_q

        return combined_fea, x_q, info, route_feat_out, route_prob_out1, route_prob_out2, prompts

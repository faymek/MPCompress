...
class MLoRE(nn.Module):
    ...
    def __init__(self, p, select_list, img_size=224, patch_size=16, in_chans=3, embed_dim=768, depth=12,
        ...
        for i_layer in range(self.num_layers):
            ...
            #self.fea_fuse.append(nn.Conv2d(embed_dim, final_embed_dim, kernel_size=1, stride=1))#,BatchNorm2d(final_embed_dim), nn.GELU(),nn.Conv2d(final_embed_dim, final_embed_dim, kernel_size=3, stride=1,padding=1)))
    ...
    def load_pretrained(self, checkpoint_path, prefix=''):
        if self.p['checkpoint'] is None:
            print('Loading Imagenet pretrained weights first...')
    ...
    def forward(self, x, episode_tasks=None, eval=True, from_feat=None):
        '''
        e.g.,
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx + 1 in self.select_list: 
                ...
                x_ = rearrange(x, 'b (h w) c -> b c h w', h=self.resolution[0], w=self.resolution[1])
                if idx+1 == 3:
                    self.feat.append(x_.detach().cpu().numpy())
                    if from_feat is not None:
                        x = rearrange(from_feat, 'b c h w -> b (h w) c', h=self.resolution[0], w=self.resolution[1])
                        x_ = from_feat
                        
                f = self.fea_fuse[il](x_)
                for task in all_tasks:
                    out_feat[task].append(f.clone())
        ...
        f = self.fea_fuse[il](x)      
        
        for task in all_tasks:
            out_feat[task].append(f.clone())
            for il in range(self.num_layers):
                last_feat[task] = last_feat[task] + (1./self.num_layers) * out_feat[task][il]

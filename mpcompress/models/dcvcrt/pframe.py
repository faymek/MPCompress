...
class Encoder(nn.Module):
    ...
    def forward_cuda(self, feature, ctx, quant_step):
        if not self.fuse_conv1_flag:
            ...
            self.conv2[0].adaptor.bias.data = (
                self.conv2[0].adaptor.bias
                + torch.matmul(
                    self.conv2[0].adaptor.weight[:, :g_ch_d, 0, 0],
                    self.conv1.bias[:, None],
                )[:, 0]
            )
            self.conv2[0].adaptor.weight.data = torch.cat(
                [fuse_weight1, fuse_weight2], dim=1
            )
...
class DMCP(DmcCompressionModel):
    def __init__(self, load_path=None):
        ...
        self.q_encoder = nn.Parameter(
            torch.ones((self.get_qp_num() + extra_qp, g_ch_d, 1, 1))
        )
        self.q_decoder = nn.Parameter(
            torch.ones((self.get_qp_num() + extra_qp, g_ch_d, 1, 1))
        )
        self.q_feature = nn.Parameter(
            torch.ones((self.get_qp_num() + extra_qp, g_ch_d, 1, 1))
        )
        self.q_recon = nn.Parameter(
            torch.ones((self.get_qp_num() + extra_qp, g_ch_recon, 1, 1))
        )
    ...
    def compress(self, x, qp):
        ...
        y_q_w_0, y_q_w_1, s_w_0, s_w_1, y_hat = self.compress_prior_2x(
    ...
    def decompress(self, bit_stream, pstate, **kwargs):
        qp = pstate["qp"]
        ...
        self.entropy_coder.set_use_two_entropy_coders(pstate["ec_part"] == 1)
        self.entropy_coder.set_stream(bit_stream)
        z_size = self.get_downsampled_shape(pstate["height"], pstate["width"], 64)

...
from mpcompress.layers.dcvc_cuda_inference import (
    build_index_dec,
    build_index_enc,
    process_with_mask,
)
...
class EntropyCoder:
    ...
    def encode_z(self, symbols, cdf_group_index, start_offset, per_channel_size):
        self.encoder.encode_z(
            symbols.to(torch.int8).cpu().numpy(),
            cdf_group_index,
            start_offset,
            per_channel_size,
        )
    ...
    def decode_and_get_y(self, indexes, cdf_group_index, device, dtype):
        rv = self.decoder.decode_and_get_y(
            indexes.to(torch.uint8).cpu().numpy(), cdf_group_index
        )
    ...
    def decode_z(self, total_size, cdf_group_index, start_offset, per_channel_size):
        self.decoder.decode_z(
            total_size, cdf_group_index, start_offset, per_channel_size
        )
...
class Bitparm(nn.Module):
    def __init__(self, qp_num, channel, final=False):
        ...
        self.final = final
        self.h = nn.Parameter(
            torch.nn.init.normal_(torch.empty([qp_num, channel, 1, 1]), 0, 0.01)
        )
        self.b = nn.Parameter(
            torch.nn.init.normal_(torch.empty([qp_num, channel, 1, 1]), 0, 0.01)
        )
...
class VbrFactorizedPrior(AEHelper, nn.Module):
    ...
    def encode_z(self, x, qp):
        _, _, H, W = x.size()
        return self.entropy_coder.encode_z(
            x.reshape(-1), self.cdf_group_index, qp * self.channel, H * W
        )

    def decode_z(self, size, qp):
        self.entropy_coder.decode_z(
            self.channel * size[0] * size[1],
...
class GaussianEncoder(AEHelper, nn.Module):
    def __init__(self):
        ...
        self.log_step_recip = 1.0 / self.log_scale_step

        self.force_zero_thres = None
        self.decode_index_cache = {}
        self.decode_zeros_cache = {}
        # self.lower_bound_scale = LowerBound(self.scale_min)
    ...
    def update(self, entropy_coder, force_zero_thres=None):
        ...
        cdf_distribution = torch.distributions.normal.Normal(0.0, scales)
        for i in range(8, 1, -1):
    ...
    def build_indexes_decoder(self, scales):
        ...
        indexes, skip_cond = build_index_dec(
            scales,
            self.scale_min,
            self.scale_max,
            self.log_scale_min,
    ...
    def decode_and_get_y(self, scales, dtype, device):
        ...
        return self.get_y(
            scales.shape, scales.numel(), dtype, device, skip_cond, indexes
        )

...
class NYUD_MT(data.Dataset):
    ...
    def __init__(self,
        ...
        _scene_gt = os.path.join(root, 'scene_names.npy')
        self.all_scenes = np.load(_scene_gt)#all data labels
        self.scenes = []#split data labels
        self.all_scene_labels = [#all label categories
        ...
        for splt in self.split:
            ...
            for ii, line in enumerate(lines):
                ...
                idx = self.all_scene_labels.index(self.all_scenes[int(line)-1])
                assert idx != -1
                self.scenes.append(int(idx))
        ...
        if self.do_scene:
            assert (len(self.images) == len(self.scenes))
    ...
    def __getitem__(self, index):
        ...
        if self.do_scene:
            sample['scene'] = self.scenes[index]

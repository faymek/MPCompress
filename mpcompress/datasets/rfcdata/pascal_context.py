...
class PASCALContext(data.Dataset):
    ...
    def __init__(self,
        ...
        self.cat_part = json.load(open(os.path.join(PROJECT_ROOT_DIR, 'db_info/pascal_part.json'), 'r'))
        ...
        self.normals = []
        if self.do_normals:
            with open(os.path.join(PROJECT_ROOT_DIR, 'db_info/nyu_classes.json')) as f:
                cls_nyu = json.load(f)
            with open(os.path.join(PROJECT_ROOT_DIR, 'db_info/context_classes.json')) as f:
    ...
    def __getitem__(self, index):
        ...
        if self.do_human_parts:
            ...
            #if str(self.im_ids[index]) == '2008_000199':
            #    cv2.imwrite('imgs/hp2008_000199.png', _human_parts)

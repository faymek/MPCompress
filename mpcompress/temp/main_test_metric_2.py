...
def lpips_frames(args):
    ...
    for filename in tqdm(os.listdir(folder1)):
        ...
        try:
            ...
            # 存储结果
            prefix_dict[prefix].append(distance.item())
    ...
    # 计算并打印每个前缀的平均值
    print("\nPer-prefix average LPIPS:")
    for prefix, distances in prefix_dict.items():
        avg = sum(distances) / len(distances)
        # print(f"{prefix}: {avg:.4f} (based on {len(distances)} images)")
        print(f"{prefix}: {avg:.4f}")

    # 计算并打印全局平均值
    if all_distances:
        global_avg = sum(all_distances) / len(all_distances)
        # print(f"\nGlobal average LPIPS: {global_avg:.4f} (based on {len(all_distances)} images)")
        print(f"\nGlobal average LPIPS: {global_avg:.4f} ")
    else:

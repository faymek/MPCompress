...
def seg_evaluate(model: torch.nn.Module, source_img_path: str, org_feature_path: str, rec_feature_path: str, image_list: list, backbone_model: torch.nn.Module):
    ...
    for image_name in tqdm(image_list):
        ...
        with torch.no_grad():
            ...
            # #/////////////////////////////////////
            # rec_feature_numpy =  rec_feature_numpy * 0.1
            # noise = np.random.rand(*rec_feature_numpy.shape).astype(np.float32)
            # std_dev = 0.5; noise = np.random.normal(loc=5, scale=std_dev, size=rec_feature_numpy.shape).astype(np.float32)
            # rec_feature_numpy = rec_feature_numpy + noise
        ...
        # ///////////////////////////////////
        # mse = (np.square((org_feature-rec_feature_numpy)/rec_feature_numpy)).mean()
        mse_list.append(mse)
...
def main_eval_only(config_path: str, backbone_checkpoint_path: str, head_checkpoint_path: str, source_img_path: str, source_split_name: str, org_feature_path: str, rec_feature_path: str):
    ...
    # Evaluate and print results
    max_v = 103.2168; min_v = -530.9767; samples = 10
    # trun_low = -530.9767; trun_high = 103.2168
    trun_low = -2; trun_high = 2
    bit_depth_all = [10, 9, 8, 7, 6, 5, 4, 3]
    quant_type_all = ['uniform']

    for quant_type in quant_type_all:
        for bit_depth in bit_depth_all:
            print(trun_low, trun_high, samples, bit_depth, quant_type)
            rec_feature_path = f"/home/gaocs/projects/FCM-LM/Data/dinov2/seg/fqa/{quant_type}_quantized/trunl{trun_low}_trunh{trun_high}/bitdepth{bit_depth}"
            ...
            # print(f"\nmIoU: {all_miou*100:.4f}")
            # print(f"Feature MSE: {np.mean(mse_list):.8f}")
            print(f"{np.mean(mse_list):.8f}", f"{all_miou*100:.4f}")

...
def vtm_encoding(
    ...
    subp.run(
        f'{vtm_path}/EncoderAppStatic -c {vtm_path}/encoder_intra_vtm.cfg -i {preprocessed_yuv_name} -o "" -b {bitstream_name} -q {qp} --ConformanceWindowMode=1 -wdt {wdt} -hgt {hgt} -f 1 -fr 1 --InternalBitDepth={bit_depth} --InputBitDepth={bit_depth} --InputChromaFormat=400 --OutputBitDepth={bit_depth}',
        stdout=stdout_vtm,
        shell=True,
    )
...
def vtm_decoding(vtm_path, bitstream_name, decoded_yuv_name, decompress_log_name):
    ...
    subp.run(
        f"{vtm_path}/DecoderAppStatic -b {bitstream_name} -o {decoded_yuv_name}",
        stdout=stdout_vtm,
        shell=True,
    )
...
def vtm_pipeline(org_feat_path, test_root, cfg, QP):
    # Set related paths
    preprocessed_yuv_path = f"{test_root}/preprocessed"
    os.makedirs(preprocessed_yuv_path, exist_ok=True)
    bitstream_path = f"{test_root}/bitstream/QP{QP}"
    os.makedirs(bitstream_path, exist_ok=True)
    decoded_yuv_path = f"{test_root}/decoded/QP{QP}"
    os.makedirs(decoded_yuv_path, exist_ok=True)
    postprocessed_feat_path = f"{test_root}/postprocessed/QP{QP}"
    os.makedirs(postprocessed_feat_path, exist_ok=True)
    encoding_log_path = f"{test_root}/encoding_log/QP{QP}"
    os.makedirs(encoding_log_path, exist_ok=True)
    decoding_log_path = f"{test_root}/decoding_log/QP{QP}"
    os.makedirs(decoding_log_path, exist_ok=True)
    ...
    for feat_name in tqdm(feat_names, total=len(feat_names), desc="Compress features"):
        ...
        org_feat_name = os.path.join(
            org_feat_path, f"{stem}.npy"
        )  # print(org_feat_name)
        preprocessed_yuv_name = os.path.join(
            preprocessed_yuv_path, f"{stem}.yuv"
        )  # print(preprocessed_yuv_name)
        bitstream_name = os.path.join(
            bitstream_path, f"{stem}.bin"
        )  # print(bitstream_name)
        decoded_yuv_name = os.path.join(
            decoded_yuv_path, f"{stem}.yuv"
        )  # print(decoded_yuv_name)
        postprocessed_feat_name = os.path.join(
            postprocessed_feat_path, f"{stem}.npy"
        )  # print(postprocessed_feat_name)
        encoding_log_name = os.path.join(
            encoding_log_path, f"{stem}.txt"
        )  # print(encoding_log_name)
        decoding_log_name = os.path.join(
            decoding_log_path, f"{stem}.txt"
        )  # print(decoding_log_name)
        ...
        # Quantization
        quant_feat = uniform_quantization(
            trun_feat, cfg.trun_low, cfg.trun_high, cfg.bit_depth
        )
        ...
        with open(decoded_yuv_name, "rb") as f:
            ...
            )  # save converted YUV file to dist
        ...
        if cfg.model_type == "sd3":
            ...
            dequant_feat = dequant_feat.astype(np.float32)
        elif cfg.model_type == "llama3":
            dequant_feat = dequant_feat.astype(np.float32)
...
def vtm_decode_only(org_feat_path, test_path, cfg, QP):
    # Set related paths
    preprocessed_yuv_path = f"{test_path}/preprocessed"
    os.makedirs(preprocessed_yuv_path, exist_ok=True)
    bitstream_path = f"{test_path}/bitstream/QP{QP}"
    os.makedirs(bitstream_path, exist_ok=True)
    decoded_yuv_path = f"{test_path}/decoded/QP{QP}"
    os.makedirs(decoded_yuv_path, exist_ok=True)
    postprocessed_feat_path = f"{test_path}/postprocessed/QP{QP}"
    os.makedirs(postprocessed_feat_path, exist_ok=True)
    encoding_log_path = f"{test_path}/encoding_log/QP{QP}"
    os.makedirs(encoding_log_path, exist_ok=True)
    decoding_log_path = f"{test_path}/decoding_log/QP{QP}"
    os.makedirs(decoding_log_path, exist_ok=True)
    ...
    for idx, feat_name in enumerate(feat_names):
        ...
        encoding_log_name = os.path.join(
            encoding_log_path, f"{stem}.log"
        )  # print(encoding_log_name)
        decoding_log_name = os.path.join(
            decoding_log_path, f"{stem}.log"
        )  # print(decoding_log_name)
...
def get_vtm_fc_config(preset_name):
    ...
    if cfg.trun_flag is False:
        config_str = (
            f"trun0_{cfg.quant_type}{cfg.quant_samples}_bitdepth{cfg.bit_depth}"
        )
    else:

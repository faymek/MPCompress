...
def eval_model(cfg):
    ...
    for x, img_meta in tqdm.tqdm(dataset):
        ...
        x_resized = Resize(model.img_size)(x)
        ...
        out_result = {
            **time_items,
            "bits": sum(bits_items.values()),
            "bpp": bpp,
            **bpp_items,
        }

        if hasattr(model, "get_feature_numel"):
            numel = model.get_feature_numel(x_resized)
        ...
        if "seg" in task_name:
            logits = seg_head.slide_predict(
                out_net["seg"],
                current_size=(x_resized.shape[2], x_resized.shape[3]),
...
""" example usage for slide inference VTM feature coding:
python examples/mpc/run_eval_mpc_ss.py \
    --config examples/mpc/config/eval_base.yaml examples/mpc/config/vtm/dino_org_slide_patch2d_small_last1_vtm.yaml \
    --checkpoint "" \
    --task voc2012_sel20_seg \
    --head voc2012_seg_small_last1 \
    --quality 1.0 \
    --cuda --recon 0 --real --verbose \
    --output_dir eval_voc2012_sel20_slide_small_last1_vtm
"""